...
export async function registerDay1(app: FastifyInstance): Promise<void> {
  app.get('/admin/day1/infra', async () => {
  ...
  app.get('/admin/day1/queues', async () => {
    const queues = await getAllQueueStats();
    return { queues };
  });

  app.post('/admin/day1/test-llm', async () => {
    if (!process.env.ANTHROPIC_API_KEY) {
      return { ok: false, error: 'ANTHROPIC_API_KEY 未配置 — 请在 .env 中填入后重启 api' };
    }
    try {
      const result = await withRun({ agent: 'day1:ping' }, async () => {

...
function levelStyle(l: 'ok' | 'warn' | 'critical') {
  ...
  return { bg: '#450a0a', border: '#1d9bf0', fg: '#fca5a5' };
}
...
function MiniTile({ label, value, accent }: { label: string; value: number | string; accent?: string }) {
  return (
    <div style={{ ...card, padding: '12px 14px', minWidth: 90, textAlign: 'center' }}>
      <div style={{ fontSize: 10, color: '#71767b', marginBottom: 4, textTransform: 'uppercase', letterSpacing: 0.4 }}>{label}</div>
      <div style={{ fontSize: 22, fontWeight: 700, color: accent ?? '#e7e9ea' }}>{value}</div>
    </div>
  );
}
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0f172a', color: '#e7e9ea', fontFamily: 'system-ui, -apple-system, PingFang SC, sans-serif' }}>
      <header style={{ background: '#020617', borderBottom: '1px solid #1e293b', padding: '0 24px', height: 52, display: 'flex', alignItems: 'center', gap: 16, position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        <span style={{ fontSize: 13, color: '#71767b' }}>联调加固 & 上线</span>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '20px 20px 60px' }}>
        ...
        {alerts && (
          <div style={{ marginBottom: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
              ...
              <span style={{ fontSize: 12, color: '#71767b' }}>检查时间: {new Date(alerts.checkedAt).toLocaleString('zh-CN')}</span>
            </div>
              ...
              {alerts.alerts.map((a, i) => {
                ...
                return (
                  <div key={i} style={{ padding: '8px 12px', borderRadius: 6, background: sty.bg, border: `1px solid ${sty.border}`, display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <span style={{ fontSize: 13, color: '#e7e9ea' }}>{a.message}</span>
                  </div>
                );
              })}
            </div>
            ...
            <tbody>
              {queues.map((q) => (
                <tr key={q.name}>
                  ...
                  <td style={{ ...td, color: q.counts.waiting > 50 ? '#fbbf24' : '#e7e9ea' }}>{q.counts.waiting}</td>
                  <td style={{ ...td, color: q.counts.active > 0 ? '#0ea5e9' : '#71767b' }}>{q.counts.active}</td>
                  <td style={{ ...td, color: '#71767b' }}>{q.counts.delayed}</td>
                  <td style={{ ...td, color: '#71767b' }}>{q.counts.completed}</td>
                  <td style={{ ...td, color: q.counts.failed > 0 ? '#fca5a5' : '#71767b', fontWeight: q.counts.failed > 0 ? 600 : 400 }}>{q.counts.failed}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {agentSummary.length === 0 ? (
                <tr><td colSpan={6} style={{ ...td, color: '#71767b', textAlign: 'center', padding: '20px 10px' }}>暂无数据</td></tr>
              ) : agentSummary.map((r) => (
                <tr key={r.agent}>
                  <td style={td}><code style={{ fontSize: 12, color: '#a5b4fc' }}>{r.agent}</code></td>
                  <td style={td}>{r.total}</td>
                  <td style={{ ...td, color: '#22c55e' }}>{r.success}</td>
                  <td style={{ ...td, color: r.failed > 0 ? '#fca5a5' : '#71767b', fontWeight: r.failed > 0 ? 600 : 400 }}>{r.failed}</td>
                  <td style={{ ...td, color: '#71767b' }}>{r.avg_latency_ms ? `${r.avg_latency_ms}ms` : '—'}</td>
        ...
        <p style={{ fontSize: 12, color: '#71767b', marginTop: -6, marginBottom: 12 }}>
        ...
        <section style={{ ...card, padding: 0, overflow: 'hidden', marginBottom: 28 }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {sources.length === 0 ? (
                <tr><td colSpan={6} style={{ ...td, color: '#71767b', textAlign: 'center', padding: '20px 10px' }}>暂无 Source</td></tr>
              ) : sources.map((s) => (
                <tr key={s.id}>
                    ...
                    {[10, 50, 100].map((pct) => (
                      <button key={pct}
                        style={{
                          ...
                          border: `1px solid ${s.grayscale_pct === pct ? '#6366f1' : '#3e4144'}`,
                          background: s.grayscale_pct === pct ? '#6366f1' : 'transparent',
                          color: s.grayscale_pct === pct ? '#fff' : '#71767b',
                        }}
        ...
        <div style={{ padding: 14, background: '#1e293b', border: '1px solid #3e4144', borderRadius: 8, fontSize: 12, color: '#71767b', lineHeight: 1.7 }}>
          <strong style={{ color: '#e7e9ea' }}>验收要点:</strong>
          ① 告警全绿 ② 队列 failed=0 ③ 点「触发全链路压测」→ 队列 waiting 数字上升再下降 ④ 灰度先设 10% 观察 1 小时再放量 100%
          <br />
          <strong style={{ color: '#e7e9ea' }}>Sentry:</strong> 设置 SENTRY_DSN 后执行 <code style={{ color: '#a5b4fc' }}>pnpm --filter @ch/api add @sentry/node</code>
          <br />
          <strong style={{ color: '#e7e9ea' }}>压测 500 条:</strong> <code style={{ color: '#a5b4fc' }}>pnpm --filter @ch/api stress</code>
        </div>
      </div>
    </div>
  );

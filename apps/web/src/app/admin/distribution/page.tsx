...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button onClick={copy}
      style={{ padding: '4px 10px', fontSize: 12, borderRadius: 4, border: '1px solid #334155', background: copied ? '#14532d' : 'transparent', cursor: 'pointer', color: copied ? '#86efac' : '#94a3b8', whiteSpace: 'nowrap' }}>
      {copied ? '已复制' : '复制文案'}
    </button>
  );
}
    ...
    <div style={{ minHeight: '100vh', background: '#0f172a', color: '#e2e8f0', fontFamily: 'system-ui, -apple-system, PingFang SC, sans-serif' }}>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '20px 20px 60px' }}>
        ...
        {!loading && tasks.length === 0 ? (
          <div style={{ ...card, color: '#64748b', textAlign: 'center', fontSize: 14, marginBottom: 32 }}>
              ...
              <tbody>
                {tasks.map((t) => (
                  <tr key={t.id}>
                    ...
                    <td style={{ ...td, color: '#cbd5e1', lineHeight: 1.5, fontSize: 12 }}>
        ...
        {daily.length > 0 && (
          <section style={{ ...card, padding: 0, overflow: 'hidden', marginBottom: 24 }}>
            <h4 style={{ margin: 0, padding: 14, fontSize: 13, color: '#cbd5e1', fontWeight: 600, borderBottom: '1px solid #334155' }}>每日趋势</h4>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  <th style={th}>日期</th>
        ...
        {topItems.length > 0 && (
          <section style={{ ...card, padding: 0, overflow: 'hidden', marginBottom: 28 }}>
            <h4 style={{ margin: 0, padding: 14, fontSize: 13, color: '#cbd5e1', fontWeight: 600, borderBottom: '1px solid #334155' }}>Top 10 文章</h4>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  <th style={th}>标题</th>
        ...
        {daily.length === 0 && !loading && (
          <div style={{ ...card, color: '#64748b', fontSize: 13, marginBottom: 28 }}>
        ...
        {showReport && report && (
          <div style={{ marginTop: 8 }}>
            ...
            <pre style={{ background: '#0f172a', border: '1px solid #334155', borderRadius: 8, padding: 20, fontSize: 13, lineHeight: 1.7, overflowX: 'auto', whiteSpace: 'pre-wrap', wordBreak: 'break-word', color: '#cbd5e1' }}>
              {report}
            </pre>
          </div>
        )}

...
//   sticky bar  #020617
//   card        #1e293b w/ border #334155
//   text        #e2e8f0 (main) · #94a3b8 (mute) · #64748b (faint)
    ...
    <div style={{ minHeight: '100vh', background: '#0f172a', color: '#e2e8f0', fontFamily: 'system-ui, -apple-system, PingFang SC, sans-serif' }}>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '20px 20px 60px' }}>
        ...
        {lastResult?.res?.stats && (
          <section style={{ ...card, marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 16, fontSize: 13, flexWrap: 'wrap', color: '#cbd5e1' }}>
              ...
              <span style={{ color: '#94a3b8' }}>清洗失败:<b>{lastResult.res.stats.cleanFail}</b></span>
        ...
        <section style={{ ...card, padding: 0, marginBottom: 16, overflow: 'hidden' }}>
          <h3 style={{ margin: 0, padding: 14, fontSize: 13, borderBottom: '1px solid #334155', color: '#cbd5e1', fontWeight: 600 }}>账号源</h3>
              ...
              {sources.map((s) => (
                <tr key={s.id}>
                  <td style={td}>
                    ...
                    <div style={{ color: '#64748b', fontSize: 11, marginTop: 2 }}>{s.external_id}</div>
                  </td>
                  <td style={td}>
                  ...
                  <td style={{ ...td, color: '#94a3b8' }}>{s.last_fetch_at ? new Date(s.last_fetch_at).toLocaleString() : '—'}</td>
        ...
        {/* Recent items */}
        <section style={{ ...card, padding: 0, marginBottom: 16, overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 14, borderBottom: '1px solid #334155' }}>
            <h3 style={{ margin: 0, fontSize: 13, color: '#cbd5e1', fontWeight: 600 }}>
              采集预览 <span style={{ color: '#64748b', fontWeight: 400, marginLeft: 6 }}>共 {itemsTotal} 条 · 第 {page}/{totalPages} 页</span>
            </h3>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}>
              <label style={{ color: '#94a3b8' }}>
                ...
                <select
                  ...
                  style={{ marginLeft: 4, padding: '3px 6px', fontSize: 12, border: '1px solid #334155', borderRadius: 4, background: '#0f172a', color: '#e2e8f0', colorScheme: 'dark' }}
                >
                ...
                <tr key={it.id}>
                  <td style={td}>
                    <a href={it.url} target="_blank" rel="noreferrer" style={{ color: '#60a5fa', textDecoration: 'none' }}>
                      {it.title || <span style={{ color: '#64748b' }}>(无标题)</span>}
                    </a>
                  </td>
                  <td style={{ ...td, color: '#94a3b8' }}>{it.source}</td>
                  <td style={td}>
                    <code style={{ fontSize: 10, padding: '1px 6px', borderRadius: 4, background: '#0f172a', color: '#a5b4fc' }}>{it.status}</code>
                  </td>
                  <td style={{ ...td, color: '#94a3b8' }}>{new Date(it.created_at).toLocaleString()}</td>
                </tr>
              ))}
              {items.length === 0 && (
                <tr>
                  <td style={{ ...td, color: '#64748b' }} colSpan={4}>暂无条目</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              ...
              {(stats?.recent_runs?.length ?? 0) === 0 && (
                <tr><td style={{ ...td, color: '#64748b' }} colSpan={6}>暂无异步执行记录 — 点击账号行的「入队」或右上角「全量采集」</td></tr>
              )}
            </tbody>
          </table>
        </section>
...
function Tile({ label, value, accent }: { label: string; value: number | string; accent?: string }) {
  return (
    <div style={card}>
      ...
      <div style={{ fontSize: 28, fontWeight: 700, marginTop: 6, color: accent ?? '#e2e8f0' }}>{value}</div>
    </div>
  );
}
